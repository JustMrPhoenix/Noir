...
function Noir.Debug(...)
	...
	if not debugInfo then debugInfo = debug.getinfo(1) end
	...
	Noir.Msg(
		"[", Color(83, 187, 208), "DEBUG", whiteColor, "](",
		Color(255, 216, 0), debugInfo.short_src:Replace(Noir.NoirLuaPath .. "/", ""),
		whiteColor, ":", Color(255, 216, 0), debugInfo.currentline, whiteColor, ") "
	)
	if isstring(args[1]) then Msg(table.remove(args, 1)) end
