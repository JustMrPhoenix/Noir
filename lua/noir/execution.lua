...
function Noir.SendCode(code, identifier, target, transferId)
	...
	if string.find(lowerCode, "%-%-full") then
		data.full = true
	end
	if string.find(lowerCode, "%-%-shallow") then
		data.shallow = true
	end
	if string.find(lowerCode, "%-%-keys") then
		data.keys = true
	end
	if string.find(lowerCode, "%-%-values") then
		data.values = true
	end
	local depthMatch = string.match(lowerCode, "%-%-depth%s+(%d+)")
	if depthMatch then
		data.depth = tonumber(depthMatch)
	end
	...
	if target == "self" then
		...
		if not done and not isstring(returns) then
			returns = Noir.Format.FormatLong(returns, 0, data)
		end
		Noir.Environment.SendMessage(me, transferId, "run", {done, returns, opts = data})
		if not done then
...
Noir.Network.StringHandlers["runCode"] = {
	...
	received = function(sender, transferId, data)
		...
		Noir.Environment.SendMessage(sender, transferId, "run", {done, returns, opts = data})
		if not done then
