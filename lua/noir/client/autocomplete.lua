...
function Autocomplete.GetValuesAndFuncs(tbl, name, scannedTbls, depth)
	...
	for k, v in pairs(tbl) do
		...
		if string.Trim(k) == "" then continue end
	...
	for fullname, t in pairs(tableScanQueue) do
		local vals, funcs = Autocomplete.GetValuesAndFuncs(t, fullname, scannedTbls, depth - 1)
	...
	for k, v in pairs(debug.getregistry()) do
		...
		if isstring(k) and istable(v) and (v.MetaID or v.MetaName or v.__tostring) then
			for key, val in pairs(v) do
				if string.Trim(key) == "" then continue end
...
function Autocomplete.GetJS(interfaceName)
	...
	local js = Format(
		[[%s.LoadAutocomplete({values: "%s", funcs: "%s"})]],
		interfaceName, table.concat(values, "|"), table.concat(funcs, "|")
	)
