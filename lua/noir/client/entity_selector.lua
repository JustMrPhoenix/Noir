...
local function drawPickModeHUD()
	...
	draw.SimpleText(
		"Click on an entity to select it | Press ESC to cancel",
		"DermaDefault", cx, 30, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER
	)
...
function EntitySelector.StartPickMode(options)
	...
	if EntitySelector.PickMode then
		EntitySelector.StopPickMode()
	end
...
function EntitySelector.Open(options)
	...
	frame.OnRemove = function()
		Highlight.Stop()
		EntitySelector.Frame = nil
		if options.onCancel and not frame.Selected then
			options.onCancel()
		end
	end
	...
	listView.OnRowSelected = function(_, idx, row)
		if IsValid(row.Entity) then Highlight.Set(row.Entity) end
	end

	listView.DoDoubleClick = function(_, idx, row)
		selectEntity(row.Entity)
	end

	listView.OnCursorExited = function()
		Highlight.Stop()
	end
...
concommand.Add("noir_entityselector", function()
	EntitySelector.Open()
end, nil, "Open the Noir Entity Selector")
