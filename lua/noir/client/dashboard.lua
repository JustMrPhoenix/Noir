...
Dashboard.Types = {
	...
	color = {
		validate = function(val)
			return IsColor(val) or (istable(val) and val.r and val.g and val.b)
		end,
	...
	entity = {
		validate = function(val)
			return val == nil or isentity(val)
		end,
		default = nil,
		persistent = false
	},
...
function Dashboard.Register(tabName, settings, options)
	...
	if not preservedRegOrder then
		Dashboard.RegistrationOrder = Dashboard.RegistrationOrder + 1
	end
...
function Dashboard.Unregister(tabName)
	...
	for fullKey, _ in pairs(Dashboard.Callbacks) do
		if string.StartWith(fullKey, tabName .. ".") then
			Dashboard.Callbacks[fullKey] = nil
		end
	end
...
function Dashboard.Get(tabName, key)
	...
	if Dashboard.Values[fullKey] ~= nil then
		return Dashboard.Values[fullKey]
	end
...
function Dashboard.Set(tabName, key, value, skipCallback)
	...
	if not skipCallback and Dashboard.Callbacks[fullKey] then
		for _, callback in ipairs(Dashboard.Callbacks[fullKey]) do
			...
			if not ok then
				Noir.Error("Dashboard callback error: ", err, "\n")
			end
		end
	end
...
function Dashboard.ResetTab(tabName)
	...
	for _, setting in ipairs(reg.settings) do
		if setting.type ~= "button" then
			Dashboard.Set(tabName, setting.key, setting.default)
		end
	end
	return true
end
	...
	for fullKey, value in pairs(Dashboard.Values) do
		...
		if reg then
			for _, setting in ipairs(reg.settings) do
				if setting.key == key then
					local typeHandler = Dashboard.Types[setting.type]
					if typeHandler and typeHandler.persistent == false then
						isPersistent = false
					end
					break
				end
...
function Dashboard.RefreshTabs()
	...
	if #sorted == 0 then
		...
		emptyPanel.PerformLayout = function(self, w, h)
			label:Center()
		end
	...
	for _, reg in ipairs(sorted) do
		...
		btn.DoClick = function()
			Dashboard.SetActiveTab(reg.name)
		end
	...
	-- Activate first tab
	if sorted[1] then
		Dashboard.SetActiveTab(sorted[1].name)
	end
end
	...
	for _, catName in ipairs(categoryOrder) do
		...
		header.Paint = function(self, w, h)
			...
			if self.IsCollapsed then
				...
				draw.NoTexture()
				surface.DrawPoly({
					{x = arrowX - 2, y = arrowY - 2},
					{x = arrowX + 6, y = arrowY - 2},
					{x = arrowX + 2, y = arrowY + 4}
				})
			end
		...
		for _, setting in ipairs(categories[catName]) do
			...
			if IsValid(row) then
				totalHeight = totalHeight + row:GetTall() + 5
			end
		end
...
function Dashboard.CreateSettingRow(parent, setting, tabName)
	...
	if setting.type ~= "color" and setting.type ~= "text" and setting.type ~= "list" then
		controlParent = topRow
	end
...
function Dashboard.CreateControl(parent, setting, tabName)
	...
	if setting.type == "bool" then
		...
		checkbox.OnChange = function(_, val)
			Dashboard.Set(tabName, setting.key, val)
		end
		return checkbox
	...
	elseif setting.type == "string" then
		...
		entry.OnChange = function()
			Dashboard.Set(tabName, setting.key, entry:GetText())
		end
		return entry
	...
	elseif setting.type == "color" then
		...
		mixer.ValueChanged = function(_, col)
			Dashboard.Set(tabName, setting.key, Color(col.r, col.g, col.b, col.a))
		end
		return mixer
	...
	elseif setting.type == "dropdown" then
		...
		combo.OnSelect = function(_, idx, text, data)
			Dashboard.Set(tabName, setting.key, data)
		end
		return combo
	...
	elseif setting.type == "keybind" then
		...
		binder.OnChange = function(_, key)
			Dashboard.Set(tabName, setting.key, key)
		end
		return binder
	...
	elseif setting.type == "button" then
		...
		btn.DoClick = function()
			if setting.callback then
				...
				if not ok then
					Noir.Error("Dashboard button callback error: ", err, "\n")
				end
			end
		end
		return btn
		...
		viewBtn.DoClick = function()
			...
			if IsValid(ent) then
				...
				timer.Simple(2, function()
					Noir.EntitySelector.StopHighlight()
				end)
			end
		end
	...
	elseif setting.type == "list" then
		...
		addBtn.DoClick = function()
			...
			if isfunction(suggestions) then
...
-- Console command
concommand.Add("noir_dashboard", function()
	Dashboard.Show()
end, nil, "Open Noir Dashboard")
